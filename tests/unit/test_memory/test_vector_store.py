...
class TestStoreWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_store_with_retry_success(self, mock_searcher, sample_view, mock_settings):
        ...
        with patch("simu_emperor.memory.vector_store.settings") as mock_cfg:
            ...
            await store._store_with_retry(
                "seg_test_0_5", "document text", {"tick": 10}, sample_view
            )

            mock_searcher.add_segments.assert_called_once_with([sample_view])

    @pytest.mark.asyncio
    async def test_store_with_retry_fails_all_attempts(
        self, mock_searcher, sample_view, mock_settings
    ):
        ...
        with patch("simu_emperor.memory.vector_store.settings") as mock_cfg:
            ...
            with patch("asyncio.sleep", fake_sleep):
                with pytest.raises(Exception, match="Error"):
                    await store._store_with_retry("seg_test_0_5", "doc", {"tick": 10}, sample_view)
    ...
    async def test_store_with_retry_succeeds_on_second_attempt(
        ...
        with patch("simu_emperor.memory.vector_store.settings") as mock_cfg:
            ...
            store = VectorStore(vector_searcher=mock_searcher)
            await store._store_with_retry("seg_test_0_5", "document", {"tick": 10}, sample_view)
...
class TestAddSegmentsWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_add_segments_with_retry_disabled(self, mock_searcher, sample_view):
        mock_emb = MagicMock()
        mock_emb.enabled = False
        ...
        with patch("simu_emperor.memory.vector_store.settings") as mock_cfg:
            ...
            store = VectorStore(vector_searcher=mock_searcher)
            await store.add_segments_with_retry([sample_view])
            mock_searcher.add_segments.assert_not_called()
...
class TestSearch:
    ...
    @pytest.mark.asyncio
    async def test_search_degradation(self, mock_searcher, sample_view, mock_settings):
    ...
    @pytest.mark.asyncio
    async def test_search_success(self, mock_searcher, sample_view, mock_settings):
...
class TestRetryFailedEmbeddings:
    ...
    @pytest.mark.asyncio
    async def test_retry_failed_embeddings_success(self, mock_searcher, sample_view, mock_settings):
        failed_records = [
            {
                "segment_id": "seg_1",
                "summary": "Summary 1",
