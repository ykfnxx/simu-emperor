...
class TestFoodProduction:
    ...
    def test_no_crops(self):
        ...
        pop = PopulationData(
            total=Decimal("100000"), growth_rate=Decimal("0.01"),
            labor_ratio=Decimal("0.5"), happiness=Decimal("0.5"),
        )
    ...
    def test_labor_shortage(self):
        ...
        agri = AgricultureData(
            crops=[CropData(crop_type=CropType.WHEAT, area_mu=Decimal("1000000"), yield_per_mu=Decimal("1.0"))],
            irrigation_level=Decimal("0"),
        )
        # required_labor = 1000000 × 0.05 = 50000
        # available_labor = 10000 × 0.5 = 5000 → modifier = 5000/50000 = 0.1
        pop = PopulationData(
            total=Decimal("10000"), growth_rate=Decimal("0.01"),
            labor_ratio=Decimal("0.5"), happiness=Decimal("0.5"),
        )
    ...
    def test_full_irrigation(self):
        """满灌溉 modifier = 1.0。"""
        agri = AgricultureData(
            crops=[CropData(crop_type=CropType.WHEAT, area_mu=Decimal("100"), yield_per_mu=Decimal("2"))],
            irrigation_level=Decimal("1"),
        )
        pop = PopulationData(
            total=Decimal("1000000"), growth_rate=Decimal("0"),
            labor_ratio=Decimal("1"), happiness=Decimal("0.5"),
        )
        result = calculate_food_production(agri, pop)
    ...
    def test_zero_irrigation(self):
        """零灌溉 modifier = 0.6。"""
        agri = AgricultureData(
            crops=[CropData(crop_type=CropType.RICE, area_mu=Decimal("1000"), yield_per_mu=Decimal("3"))],
            irrigation_level=Decimal("0"),
        )
        pop = PopulationData(
            total=Decimal("1000000"), growth_rate=Decimal("0"),
...
class TestFoodDemand:
    ...
    def test_zero_population(self):
        pop = PopulationData(
            total=Decimal("0"), growth_rate=Decimal("0"),
            labor_ratio=Decimal("0.5"), happiness=Decimal("0.5"),
        )
        cons = ConsumptionData()
        mil = MilitaryData(garrison_size=Decimal("0"), equipment_level=Decimal("0.5"), morale=Decimal("0.5"))
...
class TestFoodSurplusAndGranary:
    def test_zhili_default(self):
        ...
        surplus, new_granary, change = calculate_food_surplus_and_granary(
            Decimal("8316000"), Decimal("7950000"), Decimal("1200000"),
        )
    ...
    def test_deficit_drains_granary(self):
        surplus, new_granary, change = calculate_food_surplus_and_granary(
            Decimal("100"), Decimal("300"), Decimal("500"),
        )
...
class TestLandTaxRevenue:
    def test_zhili_default(self):
        ...
        result = calculate_land_tax_revenue(
            Decimal("8316000"), TaxationData(), Decimal("1.0"),
        )
        assert result == Decimal("249480.00")

    def test_with_tax_modifier(self):
        result = calculate_land_tax_revenue(
            Decimal("1000000"), TaxationData(land_tax_rate=Decimal("0.05")), Decimal("1.5"),
        )
...
class TestMilitaryUpkeep:
    ...
    def test_zero_garrison(self):
        mil = MilitaryData(garrison_size=Decimal("0"), equipment_level=Decimal("1"), morale=Decimal("0.5"))
        assert calculate_military_upkeep(mil) == Decimal("0")

    def test_max_equipment(self):
        """equipment_level=1 时军费增加 50%。"""
        mil = MilitaryData(
            garrison_size=Decimal("1000"), equipment_level=Decimal("1"),
            morale=Decimal("0.5"), upkeep_per_soldier=Decimal("10"),
        )
...
class TestHappinessChange:
    def test_zhili_default(self):
        ...
        result = calculate_happiness_change(
            food_prod, food_demand, p.taxation, p.military, fiscal_surplus, total_revenue,
        )
    ...
    def test_famine_negative(self):
        ...
        result = calculate_happiness_change(
            ...
            military=MilitaryData(garrison_size=Decimal("0"), equipment_level=Decimal("0.5"), morale=Decimal("0.5")),
    ...
    def test_high_tax_burden(self):
        ...
        result = calculate_happiness_change(
            ...
            taxation=TaxationData(land_tax_rate=Decimal("0.30"), commercial_tax_rate=Decimal("0.30")),
            military=MilitaryData(garrison_size=Decimal("0"), equipment_level=Decimal("0.5"), morale=Decimal("0.5")),
            fiscal_surplus=Decimal("0"),
            total_revenue=Decimal("0"),
        )
...
class TestPopulationChange:
    ...
    def test_extreme_famine(self):
        ...
        pop = PopulationData(
            total=Decimal("2600000"), growth_rate=Decimal("0.002"),
            labor_ratio=Decimal("0.55"), happiness=Decimal("0.70"),
        )
    ...
    def test_moderate_famine(self):
        ...
        pop = PopulationData(
            total=Decimal("1000000"), growth_rate=Decimal("0.002"),
            labor_ratio=Decimal("0.55"), happiness=Decimal("0.50"),
        )
        ...
        starvation_expected = Decimal("1000000") * STARVATION_MORTALITY_RATE * (FAMINE_THRESHOLD - Decimal("0.8")) / FAMINE_THRESHOLD
        assert result == -starvation_expected

    def test_slight_shortage(self):
        """ratio=0.97 → food_modifier=0.5, 增长减半。"""
        pop = PopulationData(
            total=Decimal("1000000"), growth_rate=Decimal("0.004"),
            labor_ratio=Decimal("0.55"), happiness=Decimal("0.50"),
        )
    ...
    def test_zero_demand(self):
        """需求为零 → 视为充足，正常增长。"""
        pop = PopulationData(
            total=Decimal("1000"), growth_rate=Decimal("0.004"),
            labor_ratio=Decimal("0.5"), happiness=Decimal("0.50"),
        )
    ...
    def test_growth_rate_capped(self):
        ...
        pop = PopulationData(
            total=Decimal("1000000"), growth_rate=Decimal("0.05"),
            labor_ratio=Decimal("0.5"), happiness=Decimal("0.95"),
        )
...
class TestMoraleChange:
    def test_surplus_good_equipment(self):
        """有盈余 + 好装备 → 士气上升。"""
        mil = MilitaryData(
            garrison_size=Decimal("1000"), equipment_level=Decimal("0.8"), morale=Decimal("0.5"),
        )
    ...
    def test_deficit_poor_equipment(self):
        """赤字 + 差装备 → 士气下降。"""
        mil = MilitaryData(
            garrison_size=Decimal("1000"), equipment_level=Decimal("0.2"), morale=Decimal("0.5"),
        )
...
class TestCommerceDynamics:
    def test_zhili_default(self):
        ...
        merchant_change, prosperity_change = calculate_commerce_dynamics(
            p.commerce, p.taxation, p.population.happiness,
        )
    ...
    def test_high_tax_kills_migration(self):
        ...
        commerce = CommerceData(merchant_households=Decimal("1000"), market_prosperity=Decimal("0.5"))
        taxation = TaxationData(commercial_tax_rate=Decimal("0.34"))
        merchant_change, _ = calculate_commerce_dynamics(
            commerce, taxation, Decimal("0.8"),
        )
...
class TestFiscalBalance:
    def test_zhili_default(self):
        ...
        salary, infra, levy, total_exp, surplus = calculate_fiscal_balance(
            p.administration, Decimal("225000"), Decimal("262580"),
        )
    ...
    def test_deficit(self):
        admin = AdministrationData(
            official_count=Decimal("500"), official_salary=Decimal("100"),
            infrastructure_maintenance_rate=Decimal("0.05"), infrastructure_value=Decimal("1000000"),
            court_levy_amount=Decimal("10000"),
        )
        _, _, _, total_exp, surplus = calculate_fiscal_balance(admin, Decimal("100000"), Decimal("50000"))
