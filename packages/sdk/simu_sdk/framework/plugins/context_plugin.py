...
- **需要转达给其他官员的命令**（如"转告某某做某事"）→ 先调用 `query_role_map` 查到目标 agent_id，再调用 `send_message` 实际发送消息，最后输出文字回复确认已转达。**仅口头说"我会转达"而不调用 `send_message` 是无效的，消息不会被实际传递。**

**核心原则：先执行，再回复。** 收到需要执行操作的命令时，必须先通过工具调用实际完成操作，再输出文字汇报结果。"""
