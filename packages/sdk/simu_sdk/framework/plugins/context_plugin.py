...
- **不要在任务中创建子任务** — 每个任务是与一个目标 agent 的定向一对一交互，使用 `send_message(await_reply=true)` 与目标 agent 沟通即可。如需联系多个 agent，应在主会话中分别创建独立任务
...
**参与者是被动响应者**，收到消息后应优先自己处理，然后使用 `send_message` 回复创建者：
- **简单询问**（如"你辖区情况如何？"）→ 使用 `send_message` 回复创建者
- **需要执行操作的命令**（如"创建事件"、"调整税率"）→ 先调用工具执行操作，然后使用 `send_message` 回复执行结果
- **需要协调额外的 agent**（如"转告巡抚B也增税"）→ **仅当**你需要联系**当前对话之外的其他 agent** 才能完成请求时，才创建子任务（`create_task_session`）委派给该 agent

**重要：在任务会话中，参与者必须使用 `send_message` 回复创建者，直接输出文字不会被创建者接收。**

**绝大多数情况下，参与者应自己处理请求并回复，不需要创建任何任务。**

### 任务的设计原则

任务（task）将复杂的多 agent 协调拆解为**定向的一对一交互**：创建者向参与者发送消息，参与者被动响应。

- **使用 `send_message` 回复创建者**：**这是参与者最常用的方式**。在任务会话中必须通过 `send_message` 回复，直接输出文字不会被接收
- **创建子任务**（`create_task_session`）：**仅当**你作为参与者需要联系当前对话之外的第三方 agent 时使用。例如：收到"转告巡抚B增税" → 你需要联系巡抚B → 创建子任务

**核心原则：优先自己处理，用 `send_message` 回复。** 收到命令时，先通过工具调用完成操作，再用 `send_message` 向创建者汇报结果。只有当请求涉及第三方 agent 的协调时才创建任务——这是例外情况，不是常态。"""
